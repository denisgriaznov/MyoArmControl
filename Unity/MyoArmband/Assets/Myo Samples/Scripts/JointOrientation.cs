...
public class JointOrientation : MonoBehaviour
{
    ...
    public Quaternion our = new Quaternion();
    ...
    void Update()
    {
        ...
        var prepos = _antiYaw * antiRoll * Quaternion.LookRotation(myo.transform.forward);
        if ((prepos.y < 0.2) && (prepos.y > -0.2)) transform.rotation = new Quaternion(0.0f, prepos.y, 0.0f, prepos.w);
        Debug.Log(prepos);
        our = transform.rotation;
    ...
    public void left(float force)
    {
        if (target != null)
        {
            ...
            target.GetComponent<Rigidbody>().AddForce(Vector3.left * (100.0f + 3*force));
            target.GetComponent<Box>().isCatched = false;
            StartCoroutine("Idle");
        }
    }
    public void right(float force)
    {
        if (target != null)
        {
            ...
            target.transform.SetParent(null);
            target.GetComponent<Rigidbody>().constraints = RigidbodyConstraints.None;
            target.GetComponent<Rigidbody>().AddForce(Vector3.right* (100.0f + 3 * force));
            target.GetComponent<Box>().isCatched = false;
            StartCoroutine("Idle");
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (target == null)
        {
            ...
            target.GetComponent<Box>().isCatched = true;
